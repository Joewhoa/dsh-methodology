# 委托纪律（delegate-goal）—— Goal 契约 + 回执模板

> 把任务交给子 agent / workflow / ralph 前，父 agent 先把任务压成一份**结构化契约**；子 agent 执行完回传**带证据的回执**。目标是「结果可验证、上下文可隔离」。
>
> 思路借鉴 [tt-a1i/matt-skills-with-to-goal](https://github.com/tt-a1i/matt-skills-with-to-goal) 的 `to-goal` / `spec-executor`，适配到 DSH 的 subagent / subagent_fork / ralph / workflow 机制。
...
## 二、Goal 契约模板（委托前，父 agent 把任务压成这个）
...
- 契约（Goal）是只读规划产物：不改 tracker、不建分支、不实现。
